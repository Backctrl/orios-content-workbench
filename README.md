...
OriOS 面向 DeepSeek Harness 的内容连续生产工作台。当前版本提供 dsh-worktable 多窗口工作台入口和本地文件内容仓库：

- 保留 DSH 原生侧栏和会话、任务看板、SSH、记忆系统、知识库、技能中心等功能；
- 通过 dsh-worktable 侧栏「✍️ 内容创作工作台」项目卡片进入四窗工作台（顶栏 / 总览与选题库 / 内容编辑 / 配图及预览 + 右侧对话）；
...
打开 `http://localhost:3080` 后，dsh-worktable 侧栏「工作台」出现「✍️ 内容创作工作台」项目卡片，点击即展开四窗布局（顶栏 / 总览与选题库 / 内容编辑 / 配图及预览 + 右侧对话）。插件设置位于“设置 → 插件 → 内容工作台”。如果 Web 进程已经在运行，重新添加插件后需要重启一次 `dsh web` 才会加载新的服务端 bundle；改 client bundle 则浏览器刷新（Ctrl+F5）即生效。若 Windows 启动器提示平台不支持，请在 WSL/Linux 中启动 DSH，或使用已经运行的 Web 实例。
